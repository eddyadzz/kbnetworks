  ...
  return (
    <header className={`fixed top-0 w-full z-50 transition-all duration-500 ${
      ...
      <nav className="container mx-auto px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3 group">
            ...
            <div>
              ...
              <p className="text-xs text-gray-700 dark:text-gray-400 -mt-1">Technology Solutions</p>
            </div>
          </div>
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navItems.map((item) => (
              <a
                key={item.name}
                href={item.href}
                className="relative text-gray-900 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition-all duration-300 group py-2 font-medium"
              >
                {item.name}
          ...
          <button
            ...
            className="md:hidden p-2 rounded-lg hover:bg-white/20 dark:hover:bg-gray-800 transition-colors duration-200"
          >
            {isMenuOpen ? <X className="w-6 h-6 text-gray-900 dark:text-gray-300" /> : <Menu className="w-6 h-6 text-gray-900 dark:text-gray-300" />}
          </button>
        ...
        {/* Mobile Navigation */}
        {isMenuOpen && (
          <div className="md:hidden absolute top-full left-0 right-0 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md border-b border-gray-200/50 dark:border-gray-700 shadow-lg">
            <div className="px-6 py-4 space-y-4">
              {navItems.map((item) => (
                <a
                  key={item.name}
                  href={item.href}
                  className="block text-gray-900 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-200 font-medium"
